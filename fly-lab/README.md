...
### Lucida retry (v0.5.3)
A rejected Mirror message is restored to its input. While autoplay is active, a 429 queues that exact draft until Retry-After expires instead of asking the planner to generate another message. Closing the conversation or editing the draft cancels this automatic retry. Provider error details are displayed to distinguish quota limits when the service supplies them. Planner history is bounded to the last two short excerpts. Switching to an explicitly entered own-account key uses that account's cooldown. A live probe now submits through Lucida's native UI and full prompt, in addition to the small connection test. This does not increase or bypass upstream service quotas.

Skill labels around the schematic brain report action counts, direct teacher demonstrations, graded FlyWire actions, LLM reading/writing events and real AI replies. They are stored per selected network/control mode and describe observed evidence, not mastery or neuron-level localization. Previous training checkpoints remain valid; per-skill counting starts with this version. The own-account Groq default uses the currently documented production model openai/gpt-oss-120b (Llama 3.3 is now listed as enterprise-only in Groq's model catalogue).
