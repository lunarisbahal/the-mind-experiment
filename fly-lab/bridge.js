...
(()=>{
 ...
 window.FlyGame={
  ...
  observe(){
   ...
   const modals=['docModal','riteModal','cipherModal','dlg'].filter(visible);
   const ready=!!window.Game?.running;
   return {ready,modal:!!window.Game?.modal,needsText:!!(visible('riteModal')||visible('cipherModal')),x:p.px||0,z:p.pz||0,interior:!!w.inInterior,
