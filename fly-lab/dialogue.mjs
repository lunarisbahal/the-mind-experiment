...
export class Relay {
 constructor({fetcher=(...args)=>globalThis.fetch(...args),storage=localStorage,report=()=>{},credentials=()=>null}={}){this.preferred=null;this.credentials=credentials;this.fetcher=fetcher;this.storage=storage;this.report=report;this.tail=Promise.resolve();this.backoff=60000;this.retryAt=0;this.account='shared';try{this.retryAt=Number(storage.getItem('flywire-ai-retry-at'))||0;}catch{}}
 waitError(){const e=Error('AI hattı 429 nedeniyle bekliyor'+(this.limitDetail?' ('+this.limitDetail+')':'')+' · '+Math.max(1,Math.ceil((this.retryAt-Date.now())/1000))+' saniye sonra yeniden denenecek.');e.retryAt=this.retryAt;this.report(e.message);return e;}
 ...
 async request(messages,maxTokens,signal){
  ...
  const key=this.credentials();const account=key?'own':'shared';if(this.account&&this.account!==account){this.retryAt=account==='shared'?(Number(this.storage.getItem('flywire-ai-retry-at'))||0):0;this.limitDetail='';this.backoff=60000;}this.account=account;
...
export async function decide(relay,view,memory,recent,signal){
 ...
 const raw=await relay.generate([{role:'system',content:system},{role:'user',content:JSON.stringify({screen:{panel:view.panel,text:view.text,buttons:view.buttons,field:view.field},memory,recent:recent.slice(-2).map(r=>({decision:r.decision,value:r.value,screen:r.screen?.slice(-600),text:r.text?.slice(-600)}))})}],2048,signal);return parseDecision(raw,view);
}
