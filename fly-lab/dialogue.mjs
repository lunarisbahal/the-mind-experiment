...
export async function decide(relay,view,memory,recent,signal){
 ...
 const raw=await relay.generate([{role:'system',content:system},{role:'user',content:JSON.stringify({screen:view,memory,recent:recent.slice(-2).map(r=>({decision:r.decision,value:r.value,screen:r.screen?.slice(-600),text:r.text?.slice(-600)}))})}],2048,signal);return parseDecision(raw,view);
}
