...
export async function decide(relay,view,memory,recent,signal){
 ...
 const raw=await relay.generate([{role:'system',content:system},{role:'user',content:JSON.stringify({screen:view,memory,recent:recent.slice(-5)})}],400,signal);return parseDecision(raw,view);
}
